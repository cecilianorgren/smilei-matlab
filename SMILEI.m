...
%           Bxprev = dz2 * Bz1 + dz1 * Bz2;
    ...
    function out = parse_namelist(obj)
      ...
      while ischar(tline)
        ...
        if ~isempty(tline) && ~isempty(ihash) % we have a # somewhere
          if strcmp(tline(1),'#') % skip entire line
            tline = fgetl(fidi);
            continue
          else % only save whats before the #
            fprintf(fido, '%s\n', tline(1:ihash(1)-1));            
          end
          tline = fgetl(fidi);
          continue
      ...
      buffer = fileread(tmp_file);
      

      
      % TODO: remove all lines that start with # (commented out lines)
...
%        end
    ...
    function out = n(obj,species)
      ...
      n = zeros(obj.nx,obj.ny,obj.nz,obj.nt);
      for iSpecies = species
        pop_str = obj.species{iSpecies};
        %%% Why is the charge here??????
